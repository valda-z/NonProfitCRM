  ...
  var barcode = {
    ...
    digitToCanvasRenderer: function ($container, settings, digit, hri, xi, yi, mw, mh) {
      ...
      for(var y=0; y<lines; y++){
        ...
        for(var x=0; x<columns; x++){
          if (current == digit[y][x]) {
            ...
            if (current == '1'){
                var setX = xi + (x - len) * mw;
                if (setX > $container.width()) {
                    alert('Bar code is out of screen');
                    return false;
                }
                ctx.fillRect (setX, yi + y * mh, mw * len, mh);
            }
            current = digit[y][x];
            len=1;
          }
      ...
      digitToCanvas: function ($container, settings, digit, hri) {
          ...
          if (this.bitStringTo2DArray(digit)[0].length * w > $container.width()) {
              $container.width(this.bitStringTo2DArray(digit)[0].length * w);
              x = ($container.width() / 2) - (this.bitStringTo2DArray(digit)[0].length*w/2);
          } else {
              x = ($container.data('origW') / 2) - (this.bitStringTo2DArray(digit)[0].length*w/2);
          }
